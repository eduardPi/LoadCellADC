...
1. calibrate weight scale
2. Add Wight scale for MSR3
...
String FW_Version = "v10.1.2.a4_LC_ADC_test";

#define EXP1 1
#define EXP2 2
// #define WeightToCalibrate 1395
// #define DisplayFreeHeap false
#define SPI_clk 4000000 // SPI communication clock speed in HZ
// #define uS_TO_S_FACTOR 1000000  /* Conversion factor for micro seconds to seconds */
long uS_TO_S_FACTOR = 1000000;
// #define TIME_TO_SLEEP  5        /* Time ESP32 will go to sleep (aain seconds) */
long TIME_TO_SLEEP = 0;
RTC_DATA_ATTR int bootCount = 0;

#define EEPROM_SIZE 600 // define the number of bytes you want to access
/* EEPROM mapping
  0-25 Board settings
  25
  60-79 ssid name
  ...
  250- : Pixel Mark State
  251-275 Global ID 24 byte
  275-279 Global ID 5 byte
  ...
  500-    Board_stage status (dev, qa, prod)
  501-525 GetID URL1 dev
  526-550 GetID URL2 qa
  551-575 GetID URL3 prod
  576-580 Weight scale calibration_factor
  582-585 Known Weight to calibrate
  586-590 21 bit int tare for weight scale 
...
// #define W_DOUT 17
// #define W_CLK 21
#define ESP_Reset_MISO_Mux 13
#define DRDY_PIN 19    // Data Ready / Data Out pin


HX711 scale;

float calibration_factor = 18.10; // Shelf weight
float scaleTare = 0;
int prev_scale = 0;
int curr_scale = 0;
int deltaScale = 0;
...
float WeightDeltaSum=0;
float TotalWeightCount = 0;



//************************************************

// #define WiFi_LED 17exp1_nCS
// #define COM_LED 21
...
#define S2_2 17  // GPIO 0    on board
...
// #define FET_Shift_Clk 13  // CLOCK  // SRCLK2
...
const char *ssid = "Pi-Share-Store";
const char *password = "Edid0603!";

//----------------AP Host name Network Name and password--------------------------------
const char *host_name = "esp32";
const char *host_password = "pi123456";
//------------------------------------------------------------------------------------------
int addr = 0;
...
int ESPServerPort = 80; // ESP32 Shelf server port
// int loop_couter=0;

//////////////////   Buffer  //////////////////////////
int buffer_board[120][45];
byte buffer_board_flag[120][45];
// int buffer_board_flag[120][45];
...
byte EX1_PORT0 = B00110001;
byte EX1_PORT1 = B11000000;
byte EX2_PORT0 = B00001100;
byte EX2_PORT1 = B00001111;

unsigned int A2D_VAL = 0;
unsigned long TotalPixel = 0;

unsigned int Scan_Time_Delay_usec = 30;
int SamplingValueSumPrev = 0;
int SamplingValueSum = 0;
int SamplingValueDelta = 0;
int DynSamplingDelta = 0;
int sample_mesh_Tx_duration_Time = 2000; // Tx duration Time set
unsigned long servConfigIntervalParamenter = 5000;
unsigned long servConfigIntervalParamenter2 = 120000; // Interval for keep alive Ping request 2min
unsigned long FullBoardTxParameter = 60000;          // 600000; // Interval for Full board transmission
unsigned long TxActiveUDP_Param = 30000;              // UDP Active Last period
// unsigned long servConfigIntervalParamenter2 = 10000;   // Interval for keep alive Ping request 10 sec
...
unsigned long ShelfDltaFullInterval = 0;
unsigned long TxActiveUDP_Interval = 0;
unsigned long getIDfaulseTimer = 0;
unsigned long TX_duration = 500; // UDP transmittion duration

byte Item = 3; // Shelf(0)    foot board (1)  Hook (2)  Shelf-false + WS (3)
...
bool getID = false;
bool TxDeltaFlag = false;
bool TxActiveUDP = false;
bool WSActive = true;
bool inrush=false;
bool ADCswitch=false;
...
String Scanshw;
String Header;
String Header2; // old protocol
String Header3; // New protocol
...
String ID_name;
String ID_Local_Name;
// String GetID_dev="LOCAL_IP:4000/edge";
// String GetID_dev="172.16.10.176:4000";
String GetID_dev = "whatson-admin-qa-7677jgxcxa-ew.a.run.app";
String GetID_qa = "admin-qa.pi.club";
String GetID_prod = "admin.pi.club/edge";
// String HRPrimaryServerConfig="http://localhost:1500/api/heartbeat";
String HRPrimaryServerConfig = "http://localhost:1500/api/heartbeat";
...
byte Scan_count = 0;
byte Board_prod_status = 0;
byte ShelfDltaFull = 1;
...
String MatrixConfig(int SendColumn, int SendRaw, int SendFullColumn, int SendFullRaw, int wifipwr, int RawOffset, int ColumnOffset, int Scfltr, int Httpfltr, byte Stage, int Scan_mode, String FW_ver, String Web_config_update, String net_name, String net_pswrd, String Tramsmitting_URL, String Tramsmitting_PORT, unsigned int ScanDelay_usec, bool Fet_state, int samp_offset, bool Smpl_log, String SysIDglobal, bool tx_state, String MAC, byte changeDetectTH, String Web1stConfigServer, String Web2ndConfigServer, byte HighSteadyPxl, int HighSteadyValue, int MeshEventTH, int ValueDetectTH, int ValueDetectTH_, bool PixState, bool DynamicComp, bool Tx_ActiveFlag, String LocalID)
{

  String TxActiveFlagColor;
  if (Tx_ActiveFlag)
  {
    TxActiveFlagColor = "green";
  }
  else
  {
    TxActiveFlagColor = "red";
  }
  ...
  String serverIndex =
      ...
      TxActiveFlagColor + ";"
                          ...
                          "  <h2>Dflex Board Configuration </h2>"
                          "<p2 style=\"font-size: 15px;\"> FW version: " +
      FW_ver + "</p2>"
               "<p2 style=\"font-size: 15px;padding-left: 70%;\"> FW Type: " +
      Item_web + "</p2>"
                 "  <hr>"
                 "<span class=\"dot\"></span>"
                 "  <br>"
                 ...
                 "    <p2 style=\"padding-left: 65%;font-size: 15px;font-weight: bold;\">Last update: </p2><p2 style=\"font-size: 15px;\">" +
      Web_config_update + "</p2>"
                          "   <hr>"
                          "     <form action=\"/action_page\">"
                          "<label for = \"raw\" style=\"padding-right: 30px;\">Sampling Matrix H (SR):</label>  <input type=\"number\" id=\"raw\" name=\"raw\" value=\"" +
      String(SendRaw) + "\"  min=\"0\" max=\"255\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"rawfctr\" name=\"rawfctr\" value=\"" + String(EEPROM.read(41)) + "\" min=\"0\" max=\"255\" style=\"height: 20px; width:70px;color:#db0f2b;\">"

                                                                                                                                                                                    "<label for=\"column\" style=\"padding-left: 78px;padding-right: 35px;\">Sampling Matrix W (M):</label><input type=\"number\"  id=\"column\" name=\"column\" value=\"" +
      String(SendColumn) + "\"min=\"0\" max=\"255\" style=\"height: 20px; width:70px;\"><input type=\"number\"  id=\"columnfctr\" name=\"columnfctr\" value=\"" + String(EEPROM.read(40)) + "\" min=\"0\" max=\"255\" style=\"height: 20px; width:70px; color:#db0f2b;\">"
                                                                                                                                                                                            //    "<br>"
      ...
      Fet_state_tx + " id=\"ScanState\" name=\"ScanState\"><span class=\"slider round\"></span></label>"
                     ...
                     //"<br>"
                     // "<br>"
                     "<label for=\"samp_offset\"style=\"padding-right: 90px;\" >Sampling Offset:</label><input type=\"number\"  id=\"samp_offset\" name=\"samp_offset\" value=\"" +
      String(samp_offset) + "\" min=\"0\" max=\"4000\" style=\"height: 20px; width:70px;\"><input type=\"number\"  id=\"samp_offsetfctr\" name=\"samp_offsetfctr\" value=\"" + String(EEPROM.read(298)) + "\" min=\"0\" max=\"255\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                                          ...
                                                                                                                                                                                                          "<label class=\"switch\"> <input type=\"checkbox\"" +
      Sampling_log + " id=\"Scan_log\" name=\"Scan_log\"><span class=\"slider round\"></span></label>"
                     ...
                     //    "<br>"
                     "<label for=\"httpfilter\" style=\"padding-right: 34px;\">Tx HTTP Filter above of:</label><input type=\"number\" id=\"httpfilter\" name=\"httpfilter\" value=\"" +
      String(Httpfltr) + "\" min=\"0\" max=\"255\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"httpfilterfctr\" name=\"httpfilterfctr\" value=\"" + String((EEPROM.read(47) * 50)) + "\" min=\"0\" max=\"4095\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                                         ...
                                                                                                                                                                                                         "<label class=\"switch\"> <input type=\"checkbox\"" +
      TxState + " id=\"TxState\" name=\"TxState\"><span class=\"slider round\"></span></label>"
                "<label for=\"On\" style=\"color:white;\">On</label>"
                "<br>"
                "<label for=\"scanfilter\" style=\"padding-right: 76px;\">Scan Filter below:</label>  <input type=\"number\" id=\"scanfilter\" name=\"scanfilter\"value=\"" +
      String(Scfltr) + "\" min=\"0\" max=\"1500\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"scanfilterfctr\" name=\"scanfilterfctr\" value=\"" + String((EEPROM.read(46) * 50)) + "\"  min=\"0\" max=\"1500\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        "<label class=\"switch\"> <input type=\"checkbox\"" +
      PixelState + " id=\"PixelMarkState\" name=\"PixelMarkState\"><span class=\"slider round\"></span></label>"
                   "<label for=\"PixelOn\" style=\"color:white;\">On</label>"
                   "<br>"
                   "<label for=\"MeshEventTH\" style=\"padding-right: 24px;\">Mesh Event Mark A/R TH:</label><input type=\"number\" id=\"MeshEventTH\" name=\"MeshEventTH\" value=\"" +
      String(MeshEventTH) + "\" min=\"0\" max=\"255\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"MeshEventTHfctr\" name=\"MeshEventTHfctr\" value=\"" + String(EEPROM.read(292)) + "\" min=\"0\" max=\"255\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        "<label class=\"switch\"> <input type=\"checkbox\"" +
      DynamicCompensation + " id=\"DynamicCompensation\" name=\"DynamicCompensation\"><span class=\"slider round\"></span></label>"
                            "<label for=\"DynamicCompOn\" style=\"color:white;\">On</label>"
                            "<br>"
                            "<label for=\"ValueDetectionTH\" style=\"padding-right: 67px;\">Value Detection TH:</label><input type=\"number\" id=\"ValueDetectionTH\" name=\"ValueDetectionTH\" value=\"" +
      String(ValueDetectTH) + "\" min=\"0\" max=\"5000\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"ValueDetectionTHfctr\" name=\"ValueDetectionTHfctr\" value=\"" + String(ValueDetectTH_) + "\" min=\"0\" max=\"5000\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                                                   "<label for=\"HighSteadyValue\" style=\"padding-left: 83px;padding-right: 20px;\">Max Steady Pxl / Value:</label><label for=\"\" style=\"color:white;\">" +
      String(HighSteadyPxl) + " / " + String(HighSteadyValue) + "</label>"
                                                                "<br>"
                                                                "<label for=\"ChangeDetectionTH\" style=\"padding-right: 72px;\">Pixel Detection TH:</label><input type=\"number\" id=\"ChangeDetectionTH\" name=\"ChangeDetectionTH\" value=\"" +
      String(changeDetectTH) + "\" min=\"0\" max=\"50\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"ChangeDetectionTHfctr\" name=\"ChangeDetectionTHfctr\" value=\"" + String(EEPROM.read(296)) + "\" min=\"0\" max=\"50\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                                                      "<label for=\"SystemIDgLocal\" style=\"padding-left: 83px;padding-right: 70px;\">System ID Local:</label><label for=\"SysIDLocal\" style=\"color:white;\">" +
      LocalID + "</label>"
                "<br>"
                "<label for=\"Stage\" style=\"padding-right: 160px;\">Stage:</label>  <input type=\"number\" id=\"Stage\" name=\"Stage\" value=\"" +
      String(Stage) + "\" min=\"0\" max=\"3\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"StagefctrStagefctr\" name=\"Stagefctr\" value=\"" + String(EEPROM.read(49)) + "\" min=\"0\" max=\"255\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                            //    "<br>"
                                                                                                                                                                                            "<label for=\"systemIDglobal\" style=\"padding-left: 83px;padding-right: 60px;\">System ID Global:</label><label for=\"SysID\" style=\"color:white;\">" +
      String(ID_name) + "</label>"
                        "<br>"
                        "<label for=\"wifipwr\" style=\"padding-right:97px;\">Wifi RF Power:</label> <input type=\"number\" id=\"wifipwr\" name=\"wifipwr\" value=\"" +
      String(wifipwr) + "\" min=\"0\" max=\"11\" style=\"height: 20px; width:70px;\"><input type=\"number\" id=\"wifipwrfctr\" name=\"wifipwrfctr\" value=\"" + String(EEPROM.read(50)) + "\" min=\"0\" max=\"11\" style=\"height: 20px; width:70px;color:#db0f2b;\">"
                                                                                                                                                                                          "<label for=\"MAC_ADDRESS\" style=\"padding-left: 83px;padding-right: 90px;\">System MAC:</label><label for=\"MAC\" style=\"color:white;\">" +
      MAC + "</label>"
            ...
            //    "<br>"
            "<label for=\"ScanTimeInterval\" style=\"padding-right:68px;\">Scan interval usec:</label> <input type=\"number\" id=\"ScanTimeInterval\" name=\"ScanTimeInterval\" value=\"" +
      String(ScanDelay_usec) + "\" min=\"0\" max=\"5000\" style=\"height: 20px; width:70px;\">"
                               "<br>"

                               "<label for=\"netname\" style=\"padding-right: 94px;\">Network Name:</label><input type=\"text\" id=\"netname\" name=\"netname\" value=\"" +
      String(net_name) + "\"style=\"height: 20px; width:150px;\">"
                         ...
                         "<label for=\"TransmittingURL\" style=\"padding-left: 77px; padding-right: 33px;\">UDP Tx Address:</label><input type=\"text\" id=\"TransmittingURL\" name=\"TransmittingURL\" value=\"" +
      Tramsmitting_URL + "\"style=\"height: 20px; width:150px;\">" // change UDP socket address for transmitting
                         "<br>"

                         "<label for=\"netpswrd\"style=\"padding-right: 66px;\">Network Password:</label><input type=\"text\" id=\"netpswrd\" name=\"netpswrd\" value=\"" +
      String(net_pswrd) + "\"style=\"height: 20px; width:150px;\">"
                          ...
                          "<label for=\"TransmittingPort\"style=\"padding-left: 77px; padding-right: 62px;\">UDP Tx Port:</label><input type=\"text\" id=\"TransmittingPort\" name=\"TransmittingPort\" value=\"" +
      String(Tramsmitting_PORT) + "\"style=\"height: 20px; width:150px;\">" // change UDP socket port for transmitting
                                  "<br>"
                                  "<label for=\"PrimaryServerConfig\"style=\"padding-right: 75px;\">1st Config Server:</label><input type=\"text\" id=\"PrimaryServerConfig\" name=\"PrimaryServerConfig\" value=\"" +
      String(Web1stConfigServer) + "\"style=\"height: 20px; width:530px;\">"
                                   "<br>"
                                   "<label for=\"SecondaryServerConfig\"style=\"padding-right: 70px;\">2nd Config Server:</label><input type=\"text\" id=\"SecondaryServerConfig\" name=\"SecondaryServerConfig\" value=\"" +
      String(Web2ndConfigServer) + "\"style=\"height: 20px; width:530px;\">"
...
WebServer server(ESPServerPort);
...
int SPI_IO_expander_send(byte address, byte command, byte Exp_number)
{
...
// Serial.print( "  ");
  ...
  // Serial.println( (bit_command | address), BIN);
  bit_command = 0;
  ...
  // Serial.println( bit_command, BIN);
  digitalWrite(nCS_EXP, LOW);
  SPI.transfer16(bit_command);
  digitalWrite(nCS_EXP, HIGH);
  /*
...
int IO_exp_read(byte reg, byte Exp_number)
{
  byte reg_rcv = 0;
  byte nCS_EXP=0;
  switch(Exp_number) 
  ...
  digitalWrite(nCS_EXP, LOW); // set the SS pin to LOW
...
void LED_PWR_EX1(bool state)
{
  ...
  SPI_IO_expander_send(0x02, EX1_PORT0, EXP1);
}

void LEDY_EX1(bool state)
{
  byte Regconf = B00000000;
  if (state)
  {
  ...
  SPI_IO_expander_send(0x03, EX1_PORT1, EXP1);
}

void LEDG_EX1(bool state)
{
  byte Regconf = B00000000;
  if (state)
  {
...
void IO_reg_init()
{
  ...
  SPI_IO_expander_send(0x06, 0x00, EXP1);
  SPI_IO_expander_send(0x02, EX1_PORT0, EXP1);
  SPI_IO_expander_send(0x07, 0x00 , EXP1);
  SPI_IO_expander_send(0x03, EX1_PORT1, EXP1);
  
  SPI_IO_expander_send(0x06, 0x00, EXP2);
  SPI_IO_expander_send(0x02, EX2_PORT0, EXP2);
  SPI_IO_expander_send(0x07, 0x00 , EXP2);
  SPI_IO_expander_send(0x03, EX2_PORT1, EXP2);
}

// unsigned int A2D1_sampling()
// {
...
unsigned int A2D1_sampling()
{
  byte Regconf = B11111011;

  EX2_PORT0 = EX2_PORT0 & Regconf;
  SPI_IO_expander_send(0x02, EX2_PORT0, EXP2);
...
unsigned int A2D2_sampling()
{
  byte Regconf = B11110111;
...
void IOregRead()
{
  IO_exp_read(0x00, EXP1);
  IO_exp_read(0x01, EXP1);
}
...
void MuxControl ( int y_axis, int x_axis)
{

bool MuxchangedFlag=false;
byte Regconf = 0;
//----------------------------------------------------

if (y_axis>=0 && y_axis <16) //16 elements Field 1
{
  if (x_axis==0) 
  {
    Regconf=B00001100;
...
} else if (y_axis>=16 && y_axis<32) //16 elements Field 2
{
  if (x_axis==0) 
  {
    Regconf=B00111100;
...
} else if (y_axis>=32 && y_axis<47) //15 elements Field 3
{
  if (x_axis==0) 
  {
    Regconf=B00001101;
...
} else if (y_axis>=47 && y_axis<62) //15 elements Field 4
{
  if (x_axis==0) 
  {
    Regconf=B00111101;
...
} else if (y_axis>=62 && y_axis<77) //15 elements Field 5
{
  if (x_axis==0) 
  {
    Regconf=B00001110;
...
} else if (y_axis>=77 && y_axis<92) //15 elements Field 6
{
  if (x_axis==0) 
  {
    Regconf=B00111110;
...
} else if (y_axis>=92 && y_axis<107) //15 elements Field 7
{
  if (x_axis==0) 
  {
    Regconf=B00001111;
...
} else if (y_axis>=107 && y_axis<122) //15 elements Field 8, 2Field 16
{
  if (x_axis==0) 
  {
    Regconf=B00111111;
...
{
EX2_PORT0=Regconf;
// Serial.print( y_axis + String(" ") + x_axis + String(" "));
SPI_IO_expander_send(0x02, EX2_PORT0, EXP2);
MuxchangedFlag=false;
}
...
void Unique_ID_restore()
{
  ...
  MemEnd = 24;
  addr = 251;
  for (int i = addr; i < addr + MemEnd; i++)
  {
    byte readValue = EEPROM.read(i);
    ID_name += char(readValue);
  }
  Serial.println(String("Global ID: ") + ID_name);
  // Local ID restore
  MemEnd = 5;
  addr = 275;
  for (int i = addr; i < addr + MemEnd; i++)
  {
    byte readValue = EEPROM.read(i);
    ID_Local_Name += char(readValue);
  }
  Serial.println(String("Local ID: ") + ID_Local_Name);
}
...
void Heartbeat()
{
  ...
  doc["id"] = String(ID_name);
  doc["ip"] = String(WiFi.localIP().toString().c_str());
  doc["port"] = String(ESPServerPort);
  ...
  String PostURL = String(FirstConfigURL);
  http.begin(PostURL);
  // http.begin("https://webhook.site/cd4edf12-fe07-41de-afd5-9b865430b137");
...
void handleForm()
{
  ...
  String web_systemStage = server.arg("Stage");
  String web_systemStage_fctr = server.arg("Stagefctr");
  ...
  */
  ...
  if (web_systemStage.toInt() != Board_prod_status)
  {

    EEPROM.write(500, web_systemStage.toInt());
    EEPROM.commit();
    Board_prod_status = EEPROM.read(500);
    Serial.println(String("System stage set to: ") + Board_prod_status);
    web_last_config = HIGH;
  }

  if (web_systemStage_fctr.toInt() != EEPROM.read(49))
  {

    EEPROM.write(49, web_systemStage_fctr.toInt());
    EEPROM.commit();
    Serial.println(String("System Stage factory: ") + EEPROM.read(49));
    web_last_config = HIGH;
  }
...
void HTTP_server_set()
{
  ...
  server.on("/serverIndex", HTTP_GET, []()
            {
              ...
              server.send(200, "text/html", MatrixConfig(Column, Raw, Full_mtrx_clmn, Full_mtrx_raw, EEPROM.read(23), Raw_start_offset, x_offset, Filter_scan, Filter_tx, Board_prod_status, scan_mode, FW_Version, web_update_date, ssid_reconnect, password_reconnect /*passwrd_stars*/, UDP_TX_URL, UDP_TX_Port, Scan_Time_Delay_usec, FET_flag, sampling_offset, samp_log, ID_name, HTTP_flag, WiFi.macAddress(), ChangeDetectTH, String(FirstConfigURL), SecondConfigURL, Sampling_Top_steady, SamplingValueDelta, BufferEventTH, ValueDetectTH, ValueDetectTHfctr, PixelMarkState, dyncomp, TxActiveUDP, ID_Local_Name)); });
  ...
  server.on("/action_page", HTTP_GET, []()
    ...
    server.sendHeader("Connection", "close");
     ...
     server.send(200, "text/html", MatrixConfig(Column, Raw, Full_mtrx_clmn, Full_mtrx_raw, EEPROM.read(23), Raw_start_offset , x_offset, Filter_scan, Filter_tx, Board_prod_status, scan_mode, FW_Version, web_update_date, ssid_reconnect, password_reconnect, UDP_TX_URL, UDP_TX_Port, Scan_Time_Delay_usec, FET_flag, sampling_offset, samp_log, ID_name, HTTP_flag, WiFi.macAddress(), ChangeDetectTH, FirstConfigURL, SecondConfigURL, Sampling_Top_steady, SamplingValueDelta, BufferEventTH, ValueDetectTH, ValueDetectTHfctr, PixelMarkState,dyncomp,TxActiveUDP,ID_Local_Name)); });
...
void network_connect()
{
  ...
  if (net_id == true)
  {
    ...
    for (int i = 0; i < net; i++)
    {
      ...
      if (Wifi_ssid == ssid_)
      { // check if entered network SSID is a proper network name from the list
  ...
  if (net_id == true)
  {
    ...
    if (net_id == true)
    {
      ...
      if (WiFi.isConnected())
      { // check if device is connected to AP
...
void Status()
{
  ...
  Serial.println(String("Local System ID: ") + ID_Local_Name);
  ...
  Serial.println(String("Board production stage: ") + Board_prod_status);
  ...
  Serial.println("EEPROM Cells               : " + String(EEPROM.length()));
  Serial.println("Free heap               : " + String(ESP.getFreeHeap()));
  Serial.println(String("Calibration Known weight   : ") + WeightToCalibrate);

  Serial.println(String("AP Connection SSID         : ") + WiFi.SSID());
  

  Serial.print("AP Connection BSSID        : ");
  Serial.println(WiFi.BSSIDstr()); // AP MAC address

  Serial.print("IP allotted to module      : ");
...
void reconnect()
{
  ...
  if (past_connection > 0)
  {
    ...
    */
    ...
    (Serial.println("There was no connection before"));
}
...
void PrimerServConfigRestore()
{
  ...
  for (int i = addr; i < addr + 100; i++)
  {
    ...
    Serial.print(FirstConfigURL[i - addr]);
  }
  // Serial.println();
...
void factory_reset()
{
  ...
  EEPROM.commit();
  delay(100);
  int systemStage_fctr = EEPROM.read(49);
  delay(100);
  EEPROM.write(500, systemStage_fctr);
  delay(100);
  EEPROM.commit();
  delay(100);
...
void Hard_factory_reset()
{
  ...
  delay(100);
  EEPROM.commit();
  EEPROM.write(250, 1); // Pixel Mark A/R Toggle
  delay(100);
  EEPROM.commit();
  EEPROM.write(291, 1); // Dynamic Compensation Toggle
  delay(100);
  EEPROM.commit();
  EEPROM.write(500, 0); // Production stage
  delay(100);
  EEPROM.commit();
  ...
  for (int y = addr; y < addr + 100; y++)
  {
    if (HRPrimaryServerConfig[y - addr] != EEPROM.read(y))
    {
      for (int i = 0; i < 100; i++)
      {
        EEPROM.write(addr + i, HRPrimaryServerConfig[i]);
        EEPROM.commit();
      }
...
void HTTP_ID_request()
{
  ...
  String URL;
  switch (Board_prod_status)
  {
  case 0:
    URL = "https://" + GetID_dev + "/edge/api/getId?";
    break;
  case 1:
    URL = "https://" + GetID_qa + "/edge/api/getId?";
    break;
  case 2:
    URL = "https://" + GetID_prod + "/edge/api/getId?";
    break;
  case 3:
    URL = "http://34.134.199.126:2224/api/sensor/getid?";
    break;
  }

  // String URL = "http://LOCAL_IP:4000/edge/api/getId?";
  if (Board_prod_status == 3)
  {
  ...
  {
    Message = URL + "mac=" + String(WiFi.macAddress()) + String("&type=") + Item + String("&width=") + Raw + String("&height=") + Column + String("&localip=") + WiFi.localIP().toString();
  }
  ...
  if (httpResponseCode > 0)
  {
    ...
    if ((ID_name = doc["id"].as<String>()) != "null")
    {
      // config_response = true;
      addr = 251;
      MemEnd = 24;

      for (byte i = 0; i < MemEnd; i++)
      {

        if (ID_name[i] != '"')
        {
          if (ID_name[i] != EEPROM.read(addr + i))
          {
      ...
      getID = true;
      Serial.println("get ID successful");
    }
    else
    {
      MemEnd = 24;
    ...
    if ((ID_name_Local = doc["config"]["localId"].as<String>()) != "null")
    {
      // config_response = true;
      addr = 275;
      MemEnd = 5;
      ...
      for (byte i = 0; i < MemEnd; i++)
      {
        ...
        if (ID_name_Local[i] != '"')
        {
          if (ID_name_Local[i] != EEPROM.read(addr + i))
          {
            EEPROM.write(addr + i, ID_name_Local[i]);
            EEPROM.commit();
          }
        }
        else
      ...
      Unique_ID_restore();
      Serial.println("get Local ID successful");
    }
    else
    {
      MemEnd = 5;
      ...
      Serial.println("can't get Local ID");
      // getIDfaulseTimer=millis();
      LEDY_EX1(HIGH);
      LEDG_EX1(HIGH);
    }
    if ((UDP_address = doc["config"]["udpAddr"].as<String>()) != "null")
    {
    ...
    if ((UDP_active_toggle = doc["config"]["mute"].as<String>()) != "null")
    {
      // config_response = true;
      // Serial.println (String("UDP Tx Active was to: ") + HTTP_flag);
      HTTP_flag = !UDP_active_toggle.toInt();
      TxAlways = !UDP_active_toggle.toInt();
      // Serial.println (String("UDP Tx Active set to: ") + HTTP_flag);
      if (HTTP_flag != EEPROM.read(294))
      {
        EEPROM.write(294, HTTP_flag);
    ...
    if ((PrimerConfURL = doc["config"]["heartbeatUrl"].as<String>()) != "null")
    {
      // config_response = true;
      PrimerConfURL.toCharArray(FirstConfigURL, 100); // convert string to char array
...
void HTTP_transmit()
{
  ...
  for (int v = 0; v < UDP_buffer_index + 1; v++)
  {
    ...
    {
      Message = String(Header3 + v + (UDP_buffer_index) + "," + deltaScale + "," + curr_scale + String(",!,") + Message3_HIGH + String("$"));
    // Message = String(Header3 + v + (UDP_buffer_index) + "," + deltaScale + "," + Weight + String(",!,") + Message3_HIGH + String("$"));
    }
    ...
    // http.begin("https://webhook.site/afc1457c-af89-4731-a370-936e8d38f224");
    // http.addHeader("Content-Type", "application/json");
    // //Serial.println(PostURL);
    // int httpResponseCode = http.POST(Message);
    // // int httpResponseCode = http.POST(PostURL);
...
//  Serial.println(ReadFirstTimeFlash);
  ...
  {
  Serial.println();
  Serial.println("Start first flashing");
  ...
  String UDP_String = String(udpPort);
  UDP_String.toCharArray(UDP_TX_Port, 5);
  addr = 230;
  for (int i = 0; i < 5; i++)
  {
    EEPROM.write(addr + i, UDP_TX_Port[i]);
...
bool ChangeTX(int SampledMesh, int SampledValueSum)
{
  ...
  if ((millis() - sample_mesh_Tx_duration) < sample_mesh_Tx_duration_Time)
  {
    return TxState = true;
  }
  else
...
void writeFloatToEEPROM(float value, int startAddress)
{
  const int FloatSize = 4;
  byte *p = (byte *)(void *)&value; // Treat the float as an array of bytes

  for (int i = 0; i < FloatSize; i++)
  {
    EEPROM.write(startAddress + i, *p++);
  }

  EEPROM.commit(); // Needed for ESP32
}

float readFloatFromEEPROM(int startAddress)
{
  const int FloatSize = 4;
  float value;
  byte *p = (byte *)(void *)&value; // Treat the float as an array of bytes
...
void SettingsRestore()
{
  ...
  dyncomp = EEPROM.read(291);
  Board_prod_status = EEPROM.read(500);
  Windex=readFloatFromEEPROM(576);
  EEPROM.get(582,WeightToCalibrate);
  EEPROM.get(586,Wtare);
  // calibration_factor = readFloatFromEEPROM(576);
}
...
void bufmem_sampling()
{
  ...
  for (int i = 0; i < 120; i++)
  {
    ...
    for (int k = 0; k < 45; k++)
    {
      Serial.print(String(buffer_board[i][k]) + String(","));
    }
    Serial.println();
  }
}
...
void samplingBufferFilter()
{
  for (int i = 0; i < 120; i++)
  {
    ...
    for (int k = 0; k < 45; k++)
    {
      if (i != 0 && k != 0 && buffer_board[i - 1][k - 1] == 0 && buffer_board[i - 1][k] == 0 && buffer_board[i - 1][k + 1] == 0 && buffer_board[i][k - 1] == 0 && buffer_board[i][k + 1] == 0 && buffer_board[i + 1][k - 1] == 0 && buffer_board[i + 1][k] == 0 && buffer_board[i + 1][k + 1] == 0)
        buffer_board[i][k] = 0;
    }
  }
}
...
float ScaleTare()
{
  ...
  curr_scale = scale.get_units(1);
  //   Serial.print("Raw weight: ");
  // Serial.println (curr_scale);
  // Serial.print("\t| average units:  ");
  // Serial.print((scale.get_units(4)), 2);
  // //   Serial.print("\t| average value:  ");
  // // Serial.print((scale.get_value(10)), 2);
  ...
  if (abs(curr_scale) > 50 || abs(prev_scale > 50))
  {
    deltaScale = abs(curr_scale) - abs(prev_scale);

    if (abs(deltaScale) < 50)
    {
    ...
    else
    {
      ...
      delay(150);
      curr_scale = scale.get_units(3);
      if (curr_scale < 0)
      {
        curr_scale = 0;
      }
      deltaScale = curr_scale - prev_scale;
      // Serial.print("delta test: ");
      // Serial.println(deltaScale);
    }

    if (abs(curr_scale) < 50)
    {
      curr_scale = -1;
    }
  }
...
void WScalibration()
{
  ...
  // int knownTestWeight = 231;
  int knownTestWeight = 634;
  byte *ptr_cal_factor;
  calibration_factor = 15;
  ptr_cal_factor = (byte *)&calibration_factor;

  curr_scale = scale.get_units(10);
  Serial.println("Calibration started");
  if (curr_scale > knownTestWeight)
  {
    do 
    {
    ...
    curr_scale = scale.get_units(10);
    Serial.print("factor +10: ");
    Serial.println(curr_scale);
    } while (curr_scale > knownTestWeight);
...
addr = 576;
EEPROM.put(addr, calibration_factor);
EEPROM.commit();
// for (int i = 0, addr = 55; i < 5; i++, addr++, ptr_cal_factor++)
// {
//   if (*ptr_cal_factor!=EEPROM.read(addr))
//   {
//     EEPROM.write(addr, *ptr_cal_factor);
//   }

// }

Serial.println("Calibration complete");
Serial.println(String("Calibration factor set to: ") + calibration_factor);
}
...
{
Sampling_buffer="";
for (int y = Raw_start; y < Raw + Raw_start_offset; y += pair)
{

for (int x = Column_start; x < Column + Column_start_offset; x += pair)
{


if (buffer_board[y-1][x+1]==0 && (buffer_board[y-1][x]==0) && (buffer_board[y-1][x-1]==0) && (buffer_board[y][x+1]==0) && (buffer_board[y][x-1]==0) && (buffer_board[y+1][x+1]==0) && (buffer_board[y+1][x]==0) && (buffer_board[y+1][x-1]==0)  )
{
...
void memorylist()
{
  Serial.println("Starting memory list");
...
Serial.println("Memory list close");

}

void ADC_OSC(bool state)
{
  byte Regconf = B00000000;
  if (state)
  {
...
//   float WeightDelta=0;
//   long ADCValueDigital=0;
//   // ADC_OSC(HIGH);
//   digitalWrite(LC_A2D_nCS, HIGH);
//  digitalWrite(ESP_Reset_MISO_Mux, LOW);
...
// //    delayMicroseconds(10); // Adjust delay as needed for your application
// //  }
// //  digitalWrite(LC_A2D_nCS, LOW);
//   digitalWrite(ESP_Reset_MISO_Mux, HIGH);
//   // ADC_OSC(LOW);
//   // delay(0.1);
//     ADCValueDigital = (adcData[0] << 16) | (adcData[1] << 8) | adcData[2];
//     adcValue = ADCValueDigital-Wtare;
//     Weight=adcValue/Windex;
//     // WeightDelta=Weight-PrevWeight;
//     WeightDelta=Weight-ActualPrevWeight;
//     ActualPrevWeight=Weight;
//     // if (abs(Weight)<3)
//     // {
//     //   Wtare+=(Weight*Windex);
//     // }
//     if (abs(WeightDelta)<10)
//     {
//      Serial.print(String("Prev Weight: ") +  ActualPrevWeight + String("  Actual Weight: ") + Weight + String("   Delta: ") + WeightDelta + ("   Delta Sum: ") + WeightDeltaSum + String("   ADC: ") + ADCValueDigital);
//       //  Serial.print(String("  Actual Weight: ") + Weight);
//       ActualPrevWeight=Weight;
//       // Weight-=WeightDelta;
//       WeightDeltaSum+=WeightDelta;
//       // Wtare+=(WeightDelta*Windex);
//     } else {ActualPrevWeight=Weight; }
      
//     PrevWeight=Weight;
//     Serial.println(String("     Freeze Weight: ")  +  Weight + String(" gr / Windex: ") + Windex + String(" Wtare: ") + Wtare + String("   Delta: ") + WeightDelta);

// if (abs(WeightDelta)<LoadTrigger)
// {
//  WeightDelta=0; 
// } 

// deltaScale=WeightDelta;
// curr_scale=Weight;
// //  if (abs(Weight) > LoadTrigger || abs(PrevWeight > LoadTrigger))
// //   {
// //     deltaScale = abs(Weight) - abs(PrevWeight);
...
void readADCData()
{
  ...
  if (abs(WeightDelta) < 5)
  {
    if (WeightChange)
    {
      Serial.print( String("Total weight change: ")+ TotalWeightCount +String("  Prev Weight: ") +  ActualPrevWeight + String("  Actual Weight: ") + Weight + String("   Delta: ") + WeightDelta + ("   Delta Sum: ") + WeightDeltaSum + String("    ADC: ") + ADCValueDigital + String("   Windex: ") + Windex +String("  ADC w/o tare: ") + adcValue);
      Serial.println();
    WeightChange=false;
    }
    
    //  Serial.print(String("  Actual Weight: ") + Weight);
    //  Serial.print("   ADC delta: " + String(adcValue-PrevWeight) );
    ActualPrevWeight = Weight;
     TotalWeightCount=0;
    // Weight-=WeightDelta;
    WeightDeltaSum += WeightDelta;

    // Wtare+=(WeightDelta*abs(Windex));
    //  Wtare+=(adcValue-PrevWeight);
...
// }
//Wtare=Wtare/5;
Serial.println(String("Wtare: ") + Wtare + String("AdcValue: ") + adcValue);

addr = 586;
EEPROM.put(addr, adcValue);
EEPROM.commit();
...
void WindexCalibration()
{
float KnownWeight=WeightToCalibrate;
 Serial.println("Wndex Calibration Started");

readADCData();
if (Weight>KnownWeight)
{
  while (Weight>KnownWeight)
  {
  Windex+=10;
  readADCData();
  } 
...
addr = 576;
EEPROM.put(addr, Windex);
EEPROM.commit();
Serial.println("Wndex set to: " + String(Windex));
...
delay(1000);
  ...
  pinMode(LC_A2D_nCS, OUTPUT); // set the CS pin for A2D
  digitalWrite(LC_A2D_nCS, LOW);

  pinMode(exp1_nCS, OUTPUT); // set the CS pin for I/O Expander1
  digitalWrite(exp1_nCS, HIGH);

    pinMode(exp2_nCS, OUTPUT); // set the CS pin for I/O Expander2
  digitalWrite(exp2_nCS, HIGH);

  pinMode(ESP_Reset_MISO_Mux, OUTPUT); // set the ESPreset Load cell MISO Mux controller I/O pin
  digitalWrite(ESP_Reset_MISO_Mux, HIGH);
...
// delay(1000);
  ...
  if (Serial.available() > 0)
  {
    ...
    else if (serial_read.substring(0, 6) == "stage$")
    {
      String BoardStageStatus = serial_read.substring(6);
      EEPROM.write(500, BoardStageStatus.toInt());
      EEPROM.commit();
      Board_prod_status = EEPROM.read(500);
      delay(100);
      Serial.println();
      Serial.println(String("Board stage status set to:") + Board_prod_status);
      switch (Board_prod_status)
      {
      case 0:
        Serial.println(GetID_dev);
        break;
      case 1:
        Serial.println(GetID_qa);
        break;
      case 2:
        Serial.println(GetID_prod);
        break;
      }
    }
    ...
    else if (serial_read == "txalways")
    {
      TxAlways = !TxAlways;
      Serial.println(String("TxAlways set to: ") + TxAlways);
    }
    else if (serial_read == "bufmem")
    {
      bufmem();
    }
    else if (serial_read == "bufsample")
    {
      bufmem_sampling();
    }
    ...
    else if (serial_read == "server")
    {
      ...
      Serial.println(HTTPS_ServerIP); // Print the IP to the monitor window
    }
    else if (serial_read == "tare")
    {
      TareInit();
      scale.tare();
      Serial.println("Set tare to 0");
    }
     else if (serial_read == "switch")
    {
      ADCswitch=!ADCswitch;
      if (ADCswitch)
      {
    ...
    else if (serial_read == "+" || serial_read == "g")
    {
      calibration_factor += 100;
    }
    else if (serial_read == "-" || serial_read == "b")
    {
      calibration_factor -= 100;
    }
    else if (serial_read == "+" || serial_read == "a")
    {
      calibration_factor += 10;
    }
    else if (serial_read == "-" || serial_read == "z")
    {
      calibration_factor -= 10;
    }
    else if (serial_read == "*" || serial_read == "s")
    {
      calibration_factor += 1;
    }
    else if (serial_read == "/" || serial_read == "x")
    {
      calibration_factor -= 1;
    }
    else if (serial_read == "*" || serial_read == "d")
    {
      calibration_factor += 0.5;
    }
    else if (serial_read == "/" || serial_read == "c")
    {
      calibration_factor -= 0.5;
    }
    else if (serial_read == "*" || serial_read == "f")
    {
      calibration_factor += 0.1;
    }
    else if (serial_read == "/" || serial_read == "v")
    {
      calibration_factor -= 0.1;
    }
    ...
    else if (serial_read == "wsactive")
    {

      WSActive = !WSActive;
      Serial.println(String("Weight Scale set to: ") + WSActive);
    }
    else if (serial_read == "restart")
    {

      ESP.restart();
    }
    else if (serial_read == "wscal")
    {

      // WScalibration();
       WindexCalibration();
    }
    else if (serial_read == "inrush")
    {
      inrush=!inrush;
      Serial.println(String("Inrush set to: ") + inrush);
      ...
      EEPROM.put(addr, Windex);
      EEPROM.commit();
      Serial.println("Windex set to: " + String(Windex));


    } else if (serial_read.substring(0, 7) == "calref$") {
      String KnownCalibrateRef=serial_read.substring(7);
      WeightToCalibrate=KnownCalibrateRef.toInt();
      addr = 582;
      EEPROM.put(addr, WeightToCalibrate);
      EEPROM.commit();
      Serial.println("Calibration weight set to: " + KnownCalibrateRef);
       
    } else if (serial_read == "taremem") {

      

       
        // for (int i=0;i<sizeof(Wtare);i++)
        // {
          
        // }

      addr = 586;
      EEPROM.put(addr, Wtare);
      EEPROM.commit();
      Serial.println("Wtare stored: " + Wtare);

    } else if (serial_read == "memlist") {
      memorylist();
